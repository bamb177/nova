...
_KW_DOT = ["dot", "damage over time", "continuous damage", "burn", "bleed", "poison", "지속 피해", "지속피해", "도트", "중독", "화상", "출혈"]
...
# DOT strict matcher: avoid false positives from generic '지속'(e.g., '지속시간')
_RE_DOT_STRICT = re.compile(
    r"(damage\s*over\s*time|\bdot\b|continuous\s*damage|burn|bleed|poison|지속\s*피해|도트|중독|화상|출혈)",
    re.IGNORECASE,
)
...
_RE_ENERGY_EFF = re.compile(r"(energy\s*gain\s*efficiency|에너지\s*획득\s*효율|에너지\s*획득효율)", re.IGNORECASE)

def _is_pve_excluded_opener_energy_4p(set_name: str, rune_db: dict[str, dict]) -> bool:
    """Exclude Tide-like opener sets in PVE by effect text, even if the set name was renamed (e.g., Tide -> Kappa).

    Rule: 4P effect that is limited to the first ~10 seconds AND triggers at battle start AND is about energy gain/efficiency.
    """
    key, r = _find_rune_entry(set_name, rune_db)
    if not r:
        return False
    ...
    has_10s = bool(_RE_TIDE_4P_10S.search(four))
    has_start = bool(_RE_TIDE_4P_START.search(four))
    if not (has_10s and has_start):
        return False

    tl = four.lower()
    has_energy = bool(_RE_ENERGY_EFF.search(four)) or (("energy" in tl) and ("gain" in tl or "regen" in tl)) or (("에너지" in four) and ("획득" in four or "회복" in four))
    return bool(has_energy)
...
def _make_trigger_log(detail: dict, base: dict, profile: dict, limit_per_bucket: int = 4) -> list[str]:
    ...
    def bucket_trigger_lines(bucket: str, texts: list[str]) -> list[str]:
        ...
        for label, kws in kw_defs:
            ...
            for t in texts:
                tl = t.lower()
                if (label == "DOT" and _RE_DOT_STRICT.search(t)) or (label != "DOT" and any(k in tl for k in kws)):
                    w = W.get(label, 1.0)
...
def _best_rune_builds(profile: dict, rune_db: dict[str, dict], mode: str = "pve") -> tuple[list[dict], list[str]]:
    ...
    # Fallback: if scoring filters eliminated all combinations, pick the best available 4P and 2P separately.
    fallback_used = False
    if not best and sets:
        fallback_used = True
        # pick best 4P excluding hard-excluded sets
        cand4 = [( _score_set(profile, s, 4, rune_db, tag_idx, mode=mode), s) for s in sets]
        cand4 = [(sc, s) for (sc, s) in cand4 if sc > -1e8] or cand4
        cand4.sort(key=lambda x: x[0], reverse=True)
        s4 = cand4[0][1] if cand4 else (sets[0] if sets else "")
        # pick best 2P distinct from s4
        cand2 = [( _score_set(profile, s, 2, rune_db, tag_idx, mode=mode), s) for s in sets if s != s4]
        cand2.sort(key=lambda x: x[0], reverse=True)
        s2 = cand2[0][1] if cand2 else (next((s for s in sets if s != s4), s4) if sets else "")
        best = [( (cand4[0][0] if cand4 else 0.0) + (cand2[0][0] if cand2 else 0.0), s4, s2 )]
    ...
    if fallback_used:
        rationale.append("주의: 점수 필터로 후보가 소거되어 안전장치(fallback)로 4P/2P 최상위 조합을 구성했습니다.")
    ...
    if md == "pve":
        rationale.append("PVE: '전투 시작 후 첫 10초 에너지' 계열 4세트는 장기전 평균 효율이 낮아 추천 후보에서 제외합니다.")
...
def recommend_runes(cid: str, base: dict, detail: dict, mode: str = "pve") -> dict:
    ...
    if not rune_db:
        return {"mode": "error", "error": "runes.js 파싱 실패 또는 룬 DB가 비어 있습니다.", "builds": []}
    md = (str(mode or "pve").strip().lower() or "pve")
    ...
    if isinstance(ov, dict) and ov.get("builds"):
        ...
        for b in ov.get("builds") or []:
            ...
            for it in b.get("setPlan") or []:
                ...
                if md == "pve" and pcs == 4 and _is_pve_excluded_opener_energy_4p(sname, rune_db):
                    invalid = True
                    break
    ...
    # 오버라이드가 전부 무효(예: PVE에서 Kappa 4세트 포함 등)인 경우 자동 추천으로 fallback
    # (사용자 UI에 '추천룬이 없습니다'가 뜨는 것을 방지)
    # 아래에서 auto 추천을 계속 수행한다.
